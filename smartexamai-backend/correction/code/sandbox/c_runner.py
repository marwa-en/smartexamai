"""
C-specific Docker runner for SmartExamAI.
"""
import logging
...
class CRunner(DockerRunner):
    """Executes C code inside a secure Docker container using GCC."""

    def __init__(self):
        super().__init__(image=settings.docker_image_c, language="c")
    ...
    def _prepare_workspace(self, workspace: str, code: str, exercise: Exercise, test_cases: List[TestCase]) -> None:
        if exercise.type != ExerciseType.PROGRAM:
            raise NotImplementedError("CRunner currently only supports ExerciseType.PROGRAM (stdin/stdout).")

        # 1. Write student code
        self._write_file(workspace, "student_code.c", code)
        
        # 2. Write test inputs to individual files
        for i, tc in enumerate(test_cases):
            self._write_file(workspace, f"input_{i+1}.txt", tc.input_data)
            
        # 3. Generate and write the shell harness
        harness_code = self._generate_harness(len(test_cases))
    ...
    def _generate_harness(self, num_tests: int) -> str:
        return f"""#!/bin/sh
# --- Compilation Step ---
gcc -o student_code student_code.c 2> compile_error.txt
if [ $? -ne 0 ]; then
...
while [ $i -le {num_tests} ]; do
    ...
    ./student_code < input_$i.txt > actual_$i.txt 2> error_$i.txt
    EXIT_CODE=$?
