...
class SmartExamOrchestrator:
    ...
    def _bootstrap_imports(self) -> None:
        ...
        # ⚠️ NETTOYAGE CRUCIAL : supprimer le cache des modules qui entrent
        # en conflit entre le backend (smartexamai_backend) et le pipeline IA.
        # Exemple : smartexamai_backend/models/ vs correction/code/models/
        conflicted_prefixes = (
            "services",      # backend services vs correction/redaction/services
            "models",        # backend models vs correction/code/models
            "preprocessing", # correction/code/preprocessing
            "sandbox",       # correction/code/sandbox
            "grading",       # correction/code/grading
        )
        for key in list(sys.modules.keys()):
            if key in conflicted_prefixes or any(
                key.startswith(prefix + ".") for prefix in conflicted_prefixes
            ):
                del sys.modules[key]
